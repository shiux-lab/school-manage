    ...
    public function displayAll(): void
    {
        ...
        <div class="container table-responsive">
            <table class="table text-center table-striped">
                ...
                <tr class="row">
                    <th class="col-1"><label for="all"><input type="checkbox" :checked="key" @click="allBtn()">
                            选择</label></th>
                    ...
                    <th class="col-2" @click="sort('major')"><i class="fas fa-project-diagram"></i> 专业 <i class="fas"
                                                                                                            :class="[isClick ? 'fa-angle-up' : 'fa-angle-down']"
                                                                                                            v-if="type=='major'"></i>
                    </th>
                ...
                <tr class="row" v-for="(item,index) in currentData" :key="index">
                    ...
                    <td class="col-2">
                        <button class="btn btn-sm btn-dark" @click="alter(item.id,index)">修改</button>
                        <i class="mx-2"></i>
                        <button class="btn btn-sm btn-dark" @click="delOneBtn(item.id)">删除</button>
                    </td>
                </tr>
                <tr class="row row-cols-1" v-if="list.length > pages.pageNum">
                    <td><a class="btn btn-secondary m-2 page-item"
                           :class="[index === pages.currentPage ? 'disabled' : '']" v-for="index in pages.totalPage"
                           @click="switchPage(index)">{{ index }}</a></td>
                </tr>
                <tr class="row row-cols-1">
                    <td>
                        <a class="btn btn-dark" id="add" href="javascript:" @click="addBtn()">添加成员</a>
                        <a class="btn btn-dark" id="delete" href="javascript:" v-if="list.length"
                           @click="delBtn()">删除成员</a>
